...
-- 주의: CLOB 컬럼들은 정규화 테이블로 분리됨 (ONBOARD_SESS_* 테이블 사용)
CREATE TABLE ONBOARDING_SESSION (
    SESSION_ID VARCHAR2(100) PRIMARY KEY,  -- 세션 ID (UUID 문자열)
    MEMBER_ID VARCHAR2(30),  -- 회원 ID (FK, MEMBER.MEMBER_ID 참조, NULL 허용)
    ...
    -- MAIN_SPACE: 정규화 테이블 ONBOARD_SESS_MAIN_SPACES 사용
    ...
    -- PRIORITY_LIST: 정규화 테이블 ONBOARD_SESS_PRIORITIES 사용
    BUDGET_LEVEL VARCHAR2(20),  -- Step 6: 예산 범위
    TASTE_ID NUMBER(4),  -- 매칭된 Taste ID (1-1920, FK: TASTE_CONFIG.TASTE_ID)
    -- SELECTED_CATEGORIES: 정규화 테이블 ONBOARD_SESS_CATEGORIES 사용
    -- RECOMMENDED_PRODUCTS: 정규화 테이블 ONBOARD_SESS_REC_PRODUCTS 사용
    -- RECOMMENDATION_RESULT: 제거됨 (필요시 별도 테이블로 관리)
    CREATED_AT DATE DEFAULT SYSDATE,
    UPDATED_AT DATE DEFAULT SYSDATE,
    COMPLETED_AT DATE,
    CONSTRAINT FK_SESSION_MEMBER FOREIGN KEY (MEMBER_ID) REFERENCES MEMBER(MEMBER_ID),
    CONSTRAINT FK_SESSION_TASTE FOREIGN KEY (TASTE_ID) REFERENCES TASTE_CONFIG(TASTE_ID)
);
...
CREATE INDEX IDX_SESSION_TASTE_ID ON ONBOARDING_SESSION(TASTE_ID);
