...
def main():
    ...
    print(f"  ✅ 카테고리 ({len(categories)}개): {categories}")
    ...
    for idx, row in enumerate(data, 1):
        ...
        print(f"  [{idx}/{len(data)}] taste_id {taste_id} 처리 중...", end=' ')
        
        try:
            result = analyze_case(engine, row, categories)
            ...
            before = result['products_before_count']
            after = result['products_after_count']
            excluded = result['excluded_count']
            print(f"✅ (전:{before} 후:{after} 제외:{excluded})")
        except Exception as e:
            print(f"❌ 오류: {e}")
            all_results.append({'taste_id': taste_id, 'success': False, 'error': str(e)})
    ...
    print("📋 케이스별 상세 결과")
    print("-"*80)
    print(f"{'ID':<6} {'인테리어':<25} {'가구':<20} {'전':<6} {'후':<6} {'제외':<6}")
    print("-"*80)
    
    for result in successful:
        taste_id = result.get('taste_id', '?')
        interior = (result.get('interior_style', '')[:23] + '..') if len(result.get('interior_style', '')) > 25 else result.get('interior_style', '')
        mate = (result.get('mate', '')[:18] + '..') if len(result.get('mate', '')) > 20 else result.get('mate', '')
        before = result.get('products_before_count', 0)
        after = result.get('products_after_count', 0)
        excluded = result.get('excluded_count', 0)
        
        print(f"{taste_id:<6} {interior:<25} {mate:<20} {before:<6} {after:<6} {excluded:<6}")
...
def analyze_case(engine, row, categories):
    """단일 케이스 분석"""
    ...
    products_after = [p for p in products_before if p.id not in [ep['product_id'] for ep in excluded_products]]
