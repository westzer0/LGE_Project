"""
의류관리기_SCORE 컬럼 삭제 스크립트
"""
from api.db.oracle_client import get_connection

def delete_clothing_care_column():
    """의류관리기_SCORE 컬럼 삭제"""
    try:
            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    ...
                    WHERE TABLE_NAME = 'TASTE_CONFIG' 
                      AND COLUMN_NAME LIKE '%의류관리기%'
                """)
                columns = cur.fetchall()
                
                if columns:
                    print(f"발견된 의류관리기 관련 컬럼: {[col[0] for col in columns]}")
                    
                    # 의류관리기_SCORE 컬럼 삭제
                    column_name = '"의류관리기_SCORE"'
                    print(f"\n{column_name} 컬럼 삭제 중...")
...
if __name__ == '__main__':
    print("=== 의류관리기_SCORE 컬럼 삭제 ===\n")
    delete_clothing_care_column()
    print("\n=== 완료 ===")
